...
const HIDMempoolVesting = artifacts.require("HIDMempoolVesting");
const { vesting } = require('../config');

module.exports = function(deployer) {
    const { seedAndPrivate, liquidityProvision, teamAndAdvisory, reserveFunds, bdAndPartnerShip, netOpsMining, memPoolVesting } = vesting;
    deployer.deploy(
        HIDMempoolVesting,
        HID.address,
        memPoolVesting.beneficiary,
        memPoolVesting.startTime,
        memPoolVesting.cliffDuration,
        memPoolVesting.payOutPercentage,
        memPoolVesting.payOutInterval);
