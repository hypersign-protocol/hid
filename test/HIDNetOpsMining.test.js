const HIDVesting = artifacts.require("HIDNetOpsMining");
...
contract("HIDNetOpsMining", (accounts) => {
  ...
  const initalTokenBalance = vesting.netOpsMining.totalAmountToBeVested;
  const expectedNumberOfIntervals =
    (100 * PERCENTAGE_MULTIPLIER) / vesting.netOpsMining.payOutPercentage;
  const totalStartCliffAndWaitTime =
    vesting.netOpsMining.startTime + vesting.netOpsMining.cliffDuration;
  const expectedCliffTime =
    vesting.netOpsMining.startTime + vesting.netOpsMining.cliffDuration; // in secods
  ...
  function itShouldVerifyVestingSchedule(i) {
    describe(`vesting schedule for round ${i + 1}`, async () => {
      const unlockPercentage =
        (i + 1) * vesting.netOpsMining.payOutPercentage;
      const unlockTime =
        totalStartCliffAndWaitTime + i * vesting.netOpsMining.payOutInterval;
  ...
  describe("Vesting net ops and mining", async () => {
    /**
    ...
    describe("Test constructor parameters", async () => {
      it(`intial token should be ${
        ...
        const initalTokens = convertToken(
          vesting.netOpsMining.totalAmountToBeVested
        );
      ...
      it(
        "should be able to set startTime " +
          getDateFromEpoch(vesting.netOpsMining.startTime) +
          " properly",
        async () => {
          assert.equal(
            vesting.netOpsMining.startTime,
      ...
      it(
        "should be able to set benefiricay properly " +
          vesting.netOpsMining.beneficiary +
          "  properly",
        async () => {
          assert.equal(
            vesting.netOpsMining.beneficiary,
    ...
    describe("Test fund release process", async () => {
      const unlockTime = expectedCliffTime + vesting.netOpsMining.payOutInterval;
      
      const iminDelay = 60;
      describe("Release funds during cliff @ " + getDateFromEpoch(expectedCliffTime), async () =>{
        let delayInterval = (vesting.netOpsMining.cliffDuration - iminDelay) * MILLISECONDS;
      ...
      for (i = 1; i < expectedNumberOfIntervals; i++) {
        const unlockTime = expectedCliffTime + i * vesting.netOpsMining.payOutInterval;
      
        describe(`Release funs for round ${i + 1}: @ ${getDateFromEpoch(unlockTime)}`, async () => {
          let delayInterval = vesting.netOpsMining.payOutInterval  * MILLISECONDS;
          delay(delayInterval);
